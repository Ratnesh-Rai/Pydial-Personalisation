...
class DialogueAgent(object):
    ...
    def start_call(self, session_id, domainSimulatedUsers=None, maxNumTurnsScaling=1.0,
            start_domain=None, upfront_knowledge=None):
        '''
        ...
        self.NUM_DIALOGS += 1
        self.upfront_knowledge = upfront_knowledge
        ...
        logger.debug(self.semi_belief_manager)
        logger.debug(self.upfront_knowledge)
        logger.debug(upfront_knowledge)
        state = self.semi_belief_manager.update_belief_state(ASR_obs=None, sys_act=last_sys_act,
                                                     dstring=currentDomain,
                                                     turn=self.currentTurn,hub_id = self.hub_id,
                                                     upfront_knowledge=upfront_knowledge)
        logger.debug(state)
    ...
    def continue_call(self, asr_info, domainString=None, domainSimulatedUsers=None):
        ...
        if self._increment_turn_and_check_maxTurns():
            logger.debug("Ending because of maxTurns")
            sys_act = DiaAct('bye()')
    ...
    def end_call(self, domainSimulatedUsers=None, noTraining=False):
        ...
        self.policy_manager.finalizeRecord(domainRewards = final_rewards, task=finalInfo['usermodel'])
...
#         logger.info('agent: _hand_control')
    ...
    def _evaluate_agents_turn(self, domainSimulatedUsers=None, sys_act=None, state=None):
        ...
        if self.currentTurn==0 :
            try:
                policy = Settings.config.get('policy_' + operatingDomain, 'policytype')
            except ConfigParser.NoSectionError as e:
                if self.topic_tracker.USE_SINGLE_DOMAIN:
                    policy = Settings.config.get('policy', 'policytype')
                else:
                    raise e
            if policy == 'entropy_minimization':
                self.policy_manager.record(domainString=operatingDomain, reward=0) 
                return
            else:
...
#         assert(not self.ENDING_DIALOG)
        ...
        if self.ENDING_DIALOG:  
            ...
            logger.debug("System saying bye(), due to user saying bye")
            sys_act = DiaAct('bye()')
        return sys_act
    ...
    def _agents_semo(self, sys_act):
        ...
        if self.semo_manager is not None:
            ...
            prompt_str = None 
        logger.debug("agent semo from '" +  str(sys_act) + "' to: '" + str(prompt_str) + "'")
        return prompt_str                      
...
class AgentFactory(object):
    ...
    def start_call(self, session_id, start_domain=None, upfront_knowledge=None):
        '''
        ...
        return self.agents[agent_id].start_call(session_id, start_domain=start_domain,
                upfront_knowledge=upfront_knowledge), agent_id
