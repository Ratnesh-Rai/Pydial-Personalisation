...
class ObjectiveSuccessEvaluator(Evaluator):
    ...
    @staticmethod
    def _isValidVenueStatic(name, constraints, domainString):
        constraints2 = None
        ...
        entities = Ontology.global_ontology.entity_by_features(domainString, constraints2)
        return any(entities)


    def _isValidVenue(self, name, constraints):    
        return self._isValidVenueStatic(name, constraints, self.domainString)
                            ...
                            if task is None:
                                ...
                                for req in required:
                                    ...
                                    if req.strip(" ") not in ','.join(informs[ent]): 
                                        self.outcome = False
        reward = self.outcome * self.successReward - (not self.outcome) * self.failPenalty
        logger.debug("reward is: " + str(self.outcome) + " (success) *" + str(self.successReward)\
                + " - " + (str(not self.outcome)) + "(failure) * " + str(self.failPenalty) + " = "\
                + str(reward))
        return reward
