...
class DataBase_SQLite(DataBaseINTERFACE):
    ...
    def entropy_slots(self, constraints, slots, base=None):
        """
        Returns list of slot-entropy tuples for `slots` left when applying the
        ...
        entropies = []
        for slot in slots:
            counts = np.array(self.counts_slot(slot, bits, values), dtype='float')
            probabilities = counts / counts.sum()
            entropies.append( (slot, stats.entropy(probabilities, base=base)))
        return entropies

    def max_entropy_slot(self, constraints, slots, base=None):
        """
        Returns the slot out of `slots` with the maximum entropy in the given database, when
        removing everything not adhering to `constraints`.  `base` is used as base in the
        entropy calculation and defaults to `e`.
        """
        result = max(self.entropy_slots(constraints, slots, base),
                     key=lambda x: x[1])[0]
        return result


    def counts_slot(self, slot, where_bits, values):
        try:
            if len(where_bits):
                sql_query = '''select  count(*)
                from {} 
                where '''.format(self.domain)
                sql_query += ' and '.join(where_bits)
                sql_query += ' group by {}'.format(slot)
                self.cursor.execute(sql_query, tuple(values))
            else:
                sql_query = '''select count(*)
                from {}
                group by {}'''.format(self.domain, slot)
                self.cursor.execute(sql_query)

            qry_result = self.cursor.fetchall()
            return map(lambda item: item['count(*)'], qry_result)
        except Exception as e:
