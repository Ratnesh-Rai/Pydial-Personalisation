...
#         return result
    ...
    def constraintsCanBeDiscriminated(self, constraints):
        ...
        discriminable = False
        # NOTE: why is this the case? There can be discrimination between two items, right?
        if len(entries) < 2:
            return False
        else:
            discriminating_slots = list(self.informable_slots)
            # TODO
            # remove hardcoded deletion of these slots
            if 'name' in discriminating_slots:
                discriminating_slots.remove('name') 
            if 'price' in discriminating_slots:
    ...
    def split_constraints(self, constraints):
        """
        Splits constraints in 'real' and 'dontcare' groups.
        """
        real_constraints = {}
        dontcare_slots = set()
        for slot, value, belief in constraints:
            if value != 'dontcare':
                ...
                dontcare_slots.add(slot)
        return (real_constraints, dontcare_slots)

    
    def isDiscriminable(self, constraints):
        '''
        Checks if the given constraints list returns a list of values which can be 
        discriminated between - i.e. there is a question which we could ask which 
        would give differences between the values.
        Alternative implementation to `constraintsCanBeDiscriminated` with these differences:
        * no hardcoded negative result if two options available, these can be discriminated if
        necessary
        * no hardcoded deletion of `name` and `price` slots
        '''
        (real_constraints, dontcare_slots) = self.split_constraints(constraints) 
    ...
    def max_entropy_slot(self, accepted_constraints, desired_slots):
        """
        Returns the remaining entropy in the ontology for `desired_slots`. If these are not
        provided, they are assumed to be the system requestable slots for which no constraints
        accepted yet.
        """
        return self.db.max_entropy_slot(accepted_constraints, desired_slots)
...
class FlatOntologyManager(object):
    ...
    def get_db(self, dstring):
        if dstring in self.ontologyManagers and self.ontologyManagers[dstring] is not None:
            return self.ontologyManagers[dstring].db
        else:
            raise Exception("No OntologyManager for domain '" + dstring + "'")
    ...
    def isDiscriminable(self, domainString, constraints):
        '''
        Checks if the given constraints list returns a list of values which can be 
        discriminated between - i.e. there is a question which we could ask which 
        would give differences between the values.
        ...
        * no hardcoded deletion of `name` and `price` slots
        '''
        if self.ontologyManagers[domainString] is not None:
            return self.ontologyManagers[domainString].isDiscriminable(constraints=constraints)
        return False
    ...
    def max_entropy_slot(self, domainString, accepted_constraints, desired_slots=None):
        """
        Returns the remaining entropy in the ontology for `desired_slots`. If these are not
        provided, they are assumed to be the system requestable slots for which no constraints
        accepted yet.
        """
        if self.ontologyManagers[domainString] is not None:
            accepted_constraints = {k: v[0] for k, v in accepted_constraints.iteritems()}

            (real_constraints, dontcare_slots) = self.split_constraints(accepted_constraints)
            if desired_slots == None:
                # by default, only report on the dir
                desired_slots = set(self.get_system_requestable_slots(domainString)) - set(accepted_constraints.keys())

            return self.ontologyManagers[domainString].max_entropy_slot(real_constraints,
                                                                        desired_slots)
        return False
