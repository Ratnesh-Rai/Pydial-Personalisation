...
class A2CPolicy(Policy.Policy):
    ...
    def __init__(self, in_policy_file, out_policy_file, domainString='CamRestaurants', is_training=False):
        ...
        # parameter settings
        self.use_upfront_knowledge = False
        if cfg.has_option('dqnpolicy', 'use_upfront_knowledge'):
            self.use_upfront_knowledge = cfg.getboolean('dqnpolicy', 'use_upfront_knowledge')
    ...
    def get_n_in(self, domain_string):
        if domain_string == 'CamRestaurants':
            ...
            n_in = 268 # ic340: this is wrong
        elif domain_string == 'Laptops11':
            n_in = 257
        elif domain_string is 'TV':
            n_in = 188
        elif domain_string == 'Loans':
            n_in = 129
        else:
            raise Exception('DOMAIN {} SIZE NOT SPECIFIED, PLEASE DEFINE n_in'.format(domain_string))
        
        if cfg.has_option('usermodel', 'upfrontKnowledgeLength') and self.use_upfront_knowledge:
            n_in += cfg.getint('usermodel', 'upfrontKnowledgeLength')
        return n_in
    ...
    def flatten_belief(self, belief, domainUtil, merge=False):
        ...
        if isinstance(belief, TerminalState):
            # These magic constants represent the state space size
        ...
        if self.use_upfront_knowledge:
            policyfeatures += ['upfrontKnowledge']
        ...
        for feat in policyfeatures:
            ...
            elif feat == 'upfrontKnowledge':
                logger.debug(belief['upfrontKnowledge'])
                upfront_knowledgebeliefs = map(lambda belief: belief[1], belief['upfrontKnowledge'])
                add_feature += upfront_knowledgebeliefs
            else:
                logger.error('Invalid feature name in config: ' + feat)

            flat_belief += add_feature

        logger.debug('Flat belief size {}'.format(len(flat_belief)))
        logger.debug('Belief size {}'.format(pformat(flat_belief)))
    ...
    def convertStateAction(self, state, action):
        if isinstance(state, TerminalState):
            if self.domainUtil.domainString == 'CamRestaurants':
                ...
                return [0] * self.n_in, action
            elif self.domainUtil.domainString == 'Loans':
                return [0] * self.n_in, action
            else:
                raise Exception('No n_in given')
        else:
            flat_belief = self.flatten_belief(state, self.domainUtil)
    ...
    def nextAction(self, beliefstate):
        ...
        '''
        beliefVec = self.flatten_belief(beliefstate, self.domainUtil)
