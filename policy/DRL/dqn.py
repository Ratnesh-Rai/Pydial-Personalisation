...
class DeepQNetwork(object):
    ...
    def load_network(self, load_filename):
        ...
        split = load_filename.split('.')[-3]
        if split != '0' and split[:-1] != '0p':
            try:
                self.saver.restore(self.sess, './' + load_filename)
                print("Successfully loaded from {}".format(load_filename))
            except:
                print("Could not find old network weights from {}".format(load_filename))
        else:
            print('nothing loaded in first iteration')

    def save_network(self, save_filename):
        print('Saving deepq-network... {}'.format(save_filename))
