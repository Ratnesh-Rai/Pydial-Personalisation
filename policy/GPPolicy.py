...
class GPPolicy(Policy,PolicyCommittee.CommitteeMember):
    ...
    '''
    def __init__(self, domainString, learning, sharedParams=None, policyfilesuffix=None):
        ...
        if policyfilesuffix is not None:
            inpolicyfile += policyfilesuffix
            outpolicyfile += policyfilesuffix
        logger.warning('inpolicyfile: {}'.format(inpolicyfile))
        ...
        if Settings.config.has_option('gppolicy', 'use_upfront_knowledge'):
            self.use_upfront_knowledge = Settings.config.getboolean('gppolicy', 'use_upfront_knowledge')
...
######################################################### 
    ...
    def nextAction(self, belief):
        ...
        logger.debug("Non executable: " + str(nonExecutableActions))
        executable = self._createExecutable(nonExecutableActions)
        logger.debug("Executable: " + str(nonExecutableActions))
...
#         print currentstate._bstate
        ...
        logger.debug("Selected Action: " + str(best_action))
        logger.debug("Summary Action: " + str(summaryAct))
        ...
        logger.debug("MASTER Action: " + str(masterAct))
...
class GPState(State):
    ...
    def extractSimpleBelief(self, b, replace={}):
        ...
        for elem in b['beliefs'].keys():
            if elem == 'discourseAct':
                ...
                if elem == 'name':
                    ...
                    # Additional slots for '**NONE**' and 'dontcare'
                    additionalSlots = 2
                    ...
                    if len(self._bstate['goal_'+cur_slot]) !=\
                        ...
                        logger.error("Different number of values for slot "+cur_slot+" "+str(len(self._bstate['goal_'+cur_slot]))+\
                            " in ontology "+
                            str(Ontology.global_ontology.get_len_informable_slot(self.domainString,
        ...
        if self.use_upfront_knowledge:
            self._bstate['upfrontKnowledge'] = [0] * len(b['upfrontKnowledge'])
            # TODO: extend to support certainty ranges per vector element
            for (idx, (vector, confidence)) in enumerate(b['upfrontKnowledge']):
                self._bstate['upfrontKnowledge'][idx] = confidence
