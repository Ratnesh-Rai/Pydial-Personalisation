...
class Policy(object):
    ...
    def __init__(self, domainString, learning=False, specialDomain=False): 
        ...
        :return:
        """
        logger.info('domainString:' + domainString)
        ...
        # Total number of system actions (excluding different formulations of the inform action)
        self.numActions = len(self.actions.action_names)
        logger.debug("all actions = "+str(self.actions))
        logger.debug("all action names = "+str(self.actions.action_names))
    ...
    def act_on(self, state, user_act=None):
        '''
        ...
        :param user_act: the last input from the user
        :type user_act: list of string or DiaActWithProb objects
    ...
    def finalizeRecord(self, reward, domainInControl = None, task=None):
        ...
        logger.debug('final reward: ' + str(reward))
        return
