...
#         self.lastSystemAction = None
    ...
    def act_on(self, dstring, state, user_act=None):
        '''
        ...
        if self.committees[dstring] is not None:
            ...
            systemAct = self.domainPolicies[dstring].act_on(state=state, user_act=user_act)
    ...
    def finalizeRecord(self, domainRewards, task=None):
        '''
...
#         self.prevbelief = None
    ...
    def _load_domains_policy(self, domainString=None):
        ...
        if Settings.config.has_option('policy', 'summary_belief_threshold'):
            summary_belief_threshold = Settings.config.getfloat('policy',
                    'summary_belief_threshold')
        if Settings.config.has_option('policy_'+domainString, 'summary_belief_threshold'):
            summary_belief_threshold = Settings.config.getfloat('policy_'+domainString,
                    'summary_belief_threshold')
        if Settings.config.has_option('policy_'+domainString, 'acceptable_entity_threshold'):
            acceptable_entity_threshold = Settings.config.getint('policy_'+domainString,
                    'acceptable_entity_threshold')
        else:
            acceptable_entity_threshold = None
        ...
        logger.info("Inpolicyfile {}".format(in_policy_file))
        if domainString in self.SPECIAL_DOMAINS:
            ...
            elif policy_type == 'greedyit':
                from policy import GreedyITPolicy
                self.domainPolicies[domainString] = GreedyITPolicy.GreedyITPolicy(domainString)
            elif policy_type == 'naive_database_entropy':
                from policy.NaiveDatabaseEntropyPolicy import NaiveDatabaseEntropyPolicy
                if acceptable_entity_threshold:
                    self.domainPolicies[domainString] = NaiveDatabaseEntropyPolicy(
                            domainString,
                            summary_belief_threshold,
                            acceptable_entity_threshold)
                else:
                    self.domainPolicies[domainString] = NaiveDatabaseEntropyPolicy(
                            domainString,
                            summary_belief_threshold)
            elif policy_type == 'entropy_minimization':
                from policy.EntropyMinimizationPolicy import EntropyMinimizationPolicy
                self.domainPolicies[domainString] = EntropyMinimizationPolicy(in_policy_file,
                        out_policy_file, domainString, learning)
            elif policy_type == 'random':
                from policy.RandomQuestionPolicy import RandomQuestionPolicy
                if acceptable_entity_threshold:
                    self.domainPolicies[domainString] = RandomQuestionPolicy(
                            domainString,
                            summary_belief_threshold,
                            acceptable_entity_threshold)
                else:
                    self.domainPolicies[domainString] = RandomQuestionPolicy(
                            domainString,
                            summary_belief_threshold)
            elif policy_type == 'split':
                from policy import SplitPolicy
                logger.debug(domainString)
                self.domainPolicies[domainString] = SplitPolicy.SplitPolicy(in_policy_file,
                        out_policy_file, domainString, learning, self.shared_params)
            elif policy_type == 'gp':
