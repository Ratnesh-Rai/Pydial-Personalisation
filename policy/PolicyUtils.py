...
def getGlobalAction(belief, globalact, domainString):
    ...
    if globalact == 'INFORM_REQUESTED':
        if topnamevalue != '' and topnamevalue is not None:
...
def getInformAcceptedSlotsAboutEntity(acceptanceList, ent, numFeats):
    ...
    ans = 'inform('
    try:
        nameFeat = 'name' if 'name' in ent else 'label'
        feats = {'name': ent[nameFeat]}
    except KeyError:
        feats = {'name': str(ent)[:10] + "..."}
    acceptanceKeys = acceptanceList.keys()

    maxNumFeats = float('inf')
...
def getSelect(slot, *optionnames):
    '''
    Generates question to differentiatie between possible options for a given slot.
    
    :param slot: name of slot the user is referring to
    :type slot: str
    :param *optionnames: names of all candidate values for the customer to differentiate
    :type *optionnames: str
    '''
    slot_action_pairs = [slot + '=' + option for option in optionnames]
    slot_action_pairs_str = ', '.join(slot_action_pairs)
    return 'select(' + slot_action_pairs_str + ')'

def getConfirm(**slots_and_valnames):
    '''
    Returns action that confirms a list of slot-value name pairs
    
    :param **slots_and_valnames: slot=value name pairs to be confirmed by user
    '''
    slot_value_pairs = [slot + '=' + value for slot,value in slots_and_valnames.items()]
    slot_value_pairs_str = ', '.join(slot_value_pairs)
    return 'confirm(' + slot_value_pairs_str + ')'

# TODO: implement usage of *options
def getRequest(slot, *options):
    '''
    Returns action that asks user for desired feature listing the available options if provided.

    :param slot: the feature/slot to ask the user for
    :type slot: str
    :param *options: optional parameter that lists all options the user can choose from
    :type *options: list of str
    '''
    return 'request(' + slot + ')'

# TODO: implement ReqMore with additional slot-value suggestions
def getReqMore(**slots_and_valnames):
    '''
    Returns action to request the user for anything else, optionally suggesting provided values
    for slots.

    :param **slots_and_valnames: the constraints to look for in the next iteration
    '''
    return 'reqmore()'
...
def dactBelief(belief):
    '''
    Turns belief from {slot: {valname: beliefscore}} into [(slot, valname, beliefscore)].
    '''
    l = list()
    for key, vals in belief.items():
        (valname, belief) = vals
        l.append(DactItem(key, '=', belief))
    return l

def dactItemBelief(belief):
    '''
    Turns belief from {slot: {valname: beliefscore}} into [DactItem(slot, op, valname)].
    '''
    l = list()
    for key, vals in belief.items():
        (valname, _) = vals
        l.append(DactItem(key, '=', valname))
    return l

def flattenBelief(belief):
    '''
    Turns belief from {slot: {valname: beliefscore}} into [(slot, valname, beliefscore)].
    '''
    l = list()
