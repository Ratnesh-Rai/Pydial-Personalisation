...
from ontology.FlatOntologyManager import FlatDomainOntology
...
def initialise(configId,config_file, seed, mode, trainerrorrate=None, trainsourceiteration=None,
    ...
    if isSingleDomain:
        if policytype not in ['hdc', 'random', 'naive_database_entropy',]:
            if Settings.config.has_section("policy_"+domain):
        ...
        for domain in domains:
            if policytypes[domain] not in ['hdc', 'random', 'naive_database_entropy',]:
                if Settings.config.has_section("policy_" + domain):
                    policy_dir = getRequiredDirectory("policydir", "policy_" + domain)
                else:
                    policy_dir = getRequiredDirectory("policydir", "policy")
...
def setupPolicy(domain, configId, trainerr, source_iteration,target_iteration, seed=None):
    ...
    logger.dial("inpolicyfile:"+ inpolicyfile + " outpolicyfile" + outpolicyfile)
...
def setEvalConfig(domain, configId, evalerr, ndialogs, iteration, seed=None):
    ...
    if policytype == "gp" or policytype == "split":
        if Settings.config.has_section("gpsarsa_" + domain):
            Settings.config.set("gpsarsa_" + domain, "scale", "1")
        elif Settings.config.has_section("gpsarsa"):
...
def train_command(configfile, seed=None, trainerrorrate=None,trainsourceiteration=None,
    ...
    try:
        if seed and seed.startswith('('):
            ...
            if parallel:
                Parallel(n_jobs=len(seeds))(delayed(train_command)(configfile, seed=seed,
                    trainerrorrate=trainerrorrate, trainsourceiteration=trainsourceiteration,
                    numtrainbatches=numtrainbatches, traindialogsperbatch=traindialogsperbatch,
                    traindomains=traindomains, dbprefix=dbprefix) for seed in seeds)
            else:
                for seed in seeds:
                    print('*** Seed {} ***'.format(seed))
                    train_command(configfile, seed=seed, trainerrorrate=trainerrorrate,
                            trainsourceiteration=trainsourceiteration,
                            numtrainbatches=numtrainbatches,
...
def chat_command(configfile, seed=None, trainerrorrate=None, trainsourceiteration=None):
        ...
        try:
            ...
            initialise(configId, configfile, seed, "chat", trainerrorrate=trainerrorrate,
                       trainsourceiteration=trainsourceiteration)
            logger.dial("domains" + str(domains))
            for dom in domains:
...
def dial_command(configfile, seed=None, trainerrorrate=None, trainsourceiteration=None):
        """ Run the dialoguehub according to the supplied configfile.
        """
        try:
            configId = getConfigId(configfile)
            initialise(configId, configfile, seed, "dial", trainerrorrate=trainerrorrate,
                       trainsourceiteration=trainsourceiteration)
            logger.dial("domains" + str(domains))
            for dom in domains:
                if policytypes[dom] != 'hdc':
...
def dumpactions_command(configfile, seed=None, trainerrorrate=None, trainsourceiteration=None):
        """ Dumps all master level actions, useful for populating template file.
        """
        from policy.SummaryAction import SummaryAction
        try:
            configId = getConfigId(configfile)
            initialise(configId, configfile, seed, "chat", trainerrorrate=trainerrorrate,
                       trainsourceiteration=trainsourceiteration)
            logger.dial("domains" + str(domains))
            summary_actions = {dom: SummaryAction(dom) for dom in domains}

            [logger.debug(dom + ":" + str(sa.action_names)) for dom,sa in
                    summary_actions.iteritems()]
        except clog.ExceptionRaisedByLogger:
            print "Command Aborted - see Log file for error:",logfile
            exit(0)
        except KeyboardInterrupt:
...
def templatestub_command(configfile, seed=None, trainerrorrate=None, trainsourceiteration=None):
        """ Creates templatestub.
        """
        ...
        try:
            ...
            initialise(configId, configfile, seed, "chat", trainerrorrate=trainerrorrate,
                       trainsourceiteration=trainsourceiteration)
            logger.dial("domains:" + str(domains))
            logger.dial("domain:" + str(domain))
            domains += [domain]
            ontologies = {}
            for dom in domains:
                ontology = FlatDomainOntology(dom).ontology
                ontologies[dom] = ontology
                # all infinitives to be generated
                infs = set()
                relatives = set()

                requestable_fields = ontology['requestable']
                infs |= set(requestable_fields)

                discourse_acts = [a + '() : "";' for a in ontology['discourseAct']]
                request_acts = ['request('+f+') : "";' for f in ontology['system_requestable']]
                request_acts += ['request($x) : "";']

                informable_fields = ontology['informable'].keys()
                informable_field_vars = ['$' + chr(ord('A') + i) for i in
                        range(len(informable_fields))]
                informable_field_params = [(param, param + '_V') for param in
                        informable_field_vars]
                infs |= set(informable_fields)
                permutations = [informable_field_params[:i] for i in
                        range(len(informable_field_params)+1)]
                # Templates prefer matching items by order, so first add specific case name=none
                inform_acts = ['inform(name=none, ' + ",".join(['='.join(tpl) for tpl in
                    sublist]) + ')' + ' : "' + ", ".join(['%' + tpl[0] + '_inf(' + tpl[1] + ')' for tpl in
                        sublist]) + '";'
                        for sublist in permutations]
                inform_acts += ['inform(name=$NAME, ' + ",".join(['='.join(tpl) for tpl in
                    sublist]) + ')' + ' : "' + ", ".join(['%' + tpl[0] + '_inf(' + tpl[1] + ')' for tpl in
                        sublist]) + '";'
                        for sublist in permutations]

                # TODO: add mapping/slots all possible values
                inf_acts = ['%' + inf + '_inf($V){\n\n}' for inf in infs]

                # TODO: add name? -- use _inf for informables and use templated varnames (drop
                relatives |= set(informable_fields)
                confirm_acts = ['confirm(' + ",".join(['='.join(tpl) for tpl in
                    sublist]) + ')' + ' : "' + ", ".join(['%' + tpl[0] + '_rel(' + tpl[1] + ')' for tpl in
                        sublist]) + '";'
                        for sublist in permutations]
                rel_acts = ['%' + rel + '_rel($V){\n\n}' for rel in relatives]
                print("\n".join(reduce(lambda x,y: x +y, [inf_acts, rel_acts, discourse_acts,
                    inform_acts, confirm_acts, request_acts])))
