...
class UMHdcSim(UMSimulator.UMSimulator):
    ...
    def __init__(self, domainString, max_patience = 5):
        ...
        self.rand_decision_probs = {'InformCombination':     0.6,
                       ...
                       'ThankAck2':             0.1,
                       'AffirmCombination':     1.0,
                       # Overriden by rand_decision_probs_uk
                       'UnableToAnswer':   {},
                       }
        self.upfront_knowledge = None
        self.rand_decision_probs_uk = {
                'UnableToAnswer': {}
                }
    ...
    def push_to_agenda(self, diact):
        """
        Pushes a Diact to the agenda.
        Filters out any inform() actions about which the user has not been informed yet.
        """
        # TODO: check what value to add
        dontcare = "dontcare"
        if diact.act == 'deny':
            logger.debug(diact.act)
            logger.debug(diact.items[0].slot)
        if diact.act == 'inform':
            filtered_items = filter(lambda item: item.slot not in self.rand_decision_probs['UnableToAnswer'], diact.items)
            overridden_items = []
            for item in diact.items:
                slot = item.slot
                if slot in self.rand_decision_probs['UnableToAnswer']:
                    if Settings.random.rand() < self.rand_decision_probs['UnableToAnswer'][slot]:
                        logger.debug("Override to {} for {}".format(dontcare, slot))
                        overridden_items.append((slot, dontcare))
                    else:
                        overridden_items.append((slot, item.val))
                else:
                   overridden_items.append((slot, item.val))
            diact.items = filtered_items
            for (slot, value) in overridden_items:
                diact.append(slot, value)
        elif diact.act == 'deny':
            slot = diact.items[0].slot
            val = diact.items[0].val
            if Settings.random.rand() < self.rand_decision_probs['UnableToAnswer'][slot]:
                logger.debug("Override from deny to negate for {}".format(slot))
                diaact_str = 'negate(' + slot + '=' + val +')'
                diact = DiaAct.DiaAct(diaact_str)
        self.agenda.push(diact)
    ...
    def init(self, goal, um_patience):
        ...
        # Then create the upfront knowledge vector, uniformly
        if 'vectors' in self.rand_decision_probs_uk:
            uk_idx = Settings.random.randint(len(self.rand_decision_probs_uk['vectors']))
            confidence = Settings.random.uniform(
                    self.rand_decision_probs_uk['confidence'][0],
                    self.rand_decision_probs_uk['confidence'][1]
                    )
            self.upfront_knowledge = []
            for i in range(len(self.rand_decision_probs_uk['vectors'])):
                if i == uk_idx:
                    c = confidence
                else:
                    c = (1 - confidence) / (len(self.rand_decision_probs_uk['vectors']) - 1)
                self.upfront_knowledge.append((self.rand_decision_probs_uk['vectors'][i], c))
            self.sampling_probs['UnableToAnswer'] = self.rand_decision_probs_uk['UnableToAnswer'][uk_idx]

        if self.sampling_probs:
            self._sampleProbs()

        logger.debug(goal)
        logger.debug(type(goal.constraints[0]))
        logger.debug(self.sampling_probs['UnableToAnswer'])
        logger.debug(self.upfront_knowledge)
        new_constraints = []
        for c, constraint in enumerate(goal.constraints):
            if constraint.slot in self.sampling_probs['UnableToAnswer']:
                lb, ub = self.sampling_probs['UnableToAnswer'][constraint.slot]
                if ub == 1.0:
                    pass
                else:
        ...
        goal.constraints = new_constraints

        self.agenda.init(goal)
        logger.debug(self.agenda)
    ...
    def receive(self, sys_act, goal):
        ...
        if goal.is_completed() and self.agenda.size() == 0 and sys_act.act != 'reqmore'\
            ...
            self.agenda.clear()
            self.push_to_agenda(DiaAct.DiaAct('bye()'))
            return
        ...
        if goal.patience > 1 and sys_act.act != 'repeat' and sys_act.act != 'badact' and\
            ...
            if Settings.random.rand() < self.rand_decision_probs['Repeat']:
                self.push_to_agenda(DiaAct.DiaAct('repeat()'))
                return
        ...
        if Settings.random.rand() < self.rand_decision_probs['NullResp']:
            self.push_to_agenda(DiaAct.DiaAct('null()'))
            return
...
#             print "dialogue act", dap.act, dap.items
            ...
            while self.agenda.size() > 0 and \
                ...
                logger.debug(self.rand_decision_probs['InformCombination'])
                if Settings.random.rand() < self.rand_decision_probs['InformCombination']:
                    inf_comb_count += 1
                    next_dap = self.agenda.pop()
                    for dip in next_dap.items:
                        val = dip.val
                        if dip.slot in self.rand_decision_probs['UnableToAnswer']:
                            if Settings.random.rand() < self.rand_decision_probs['UnableToAnswer']:
                                val = "dontcare"
                        dap.append(dip.slot, val, dip.op == '!=')
                else:
                    break
    ...
    def _receive_reqmore(self, sys_act, goal):
        ...
        if goal.are_all_requests_filled():
            ...
            if Settings.random.rand() < self.rand_decision_probs['NoAfterReqmore']:
                ...
                self.push_to_agenda(DiaAct.DiaAct('negate()'))
            else:
            ...
            if goal.is_venue_recommended():
                ...
                for info in unsatisfied:
                    self.push_to_agenda(DiaAct.DiaAct('request(%s)' % info))
            ...
            # Make the system loose a point by answering "yes!"
                ...
                if Settings.random.rand() < self.rand_decision_probs['YesAfterReqmore']:
                    ...
                    self.push_to_agenda(DiaAct.DiaAct('affirm()'))
                else:
        ...
        if self.agenda.size() > 0:
            ...
            if top_item.act == 'inform':
                if Settings.random.rand() < self.rand_decision_probs['AffirmCombination']:
                    ...
                    self.agenda.pop()
        self.push_to_agenda(new_affirm_act)
    ...
    def _receive_select(self, sys_act, goal):
        ...
        if slot == 'name':
            self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (slot, value)))
        ...
        if not goal.contains_slot_const(slot):
            ...
            self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (slot, 'dontcare')))
        else:
            correct_val = goal.get_correct_const_value(slot)
            self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (slot, correct_val)))
            return
    ...
    def _receive_request(self, sys_act, goal):
        ...
        if 'name' in goal.requests and goal.requests['name'] is not None:
            for info in goal.requests:
                if goal.requests[info] is None:
                    self.push_to_agenda(DiaAct.DiaAct('request(name="%s",%s)' % (goal.requests['name'], info)))
                    return
        ...
        if requested_slot in ['venue', 'task', 'type']:
            ...
            if self.agenda.contains_act('request'):
                ...
                return
            self.push_to_agenda(DiaAct.DiaAct('inform(type="%s")' % goal.request_type))
            return
        ...
        for agenda_act in inform_items:
            for item in agenda_act.items:
                if item.slot in answer_slots:
                    ...
                    self.push_to_agenda(agenda_act)
        ...
        if f < self.rand_decision_probs['NewRequestResp1']:
            ...
            if len(goal.constraints) == 0:
                ...
                self.push_to_agenda(DiaAct.DiaAct('inform(=dontcare)'))
                ...
                self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (sampled_slot, sampled_value)))
            ...
            if random_val is None:
                ...
                if random_val is None:
                    ...
                    random_val = 'dontcare'
                    self.push_to_agenda(DiaAct.DiaAct('inform(=%s)' % random_val ))
                    ...
                    for slot in additional_slots:
                        ...
                        self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (slot, rval)))
            else:
                goal.add_const(slot=requested_slot,value=random_val) 
                #goal.constraints[requested_slot] = random_val
                self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (requested_slot, random_val)))
            ...
            self.push_to_agenda(DiaAct.DiaAct('inform(%s="%s")' % (requested_slot, 'dontcare')))
    ...
    def _receive_inform(self, sys_act, goal):
            ...
            if contains_name_none:
                ...
                if f < self.rand_decision_probs['ReqAltsAfterVenRec1']:
                    ...
                    try:
                        ...
                        self.push_to_agenda(DiaAct.DiaAct('reqalts()'))
                    except Exception, e:
                        logger.warning(e)
                    ...
                    if relax_slot is not None:
                        ...
                        if not contains_name_none:
                            if Settings.random.rand() < self.rand_decision_probs['TellAboutChange']:
                                ...
                                self.push_to_agenda(DiaAct.DiaAct('reqalts(%s="%s")' % (relax_slot, relax_value)))
                            else:
        ...
        if self.relax_constraints:
            ...
            if Settings.random.rand() < self.rand_decision_probs['ByeOrStartOver']:
                ...
                #self.push_to_agenda(DiaAct.DiaAct('inform(type=restaurant)'))
                self.push_to_agenda(DiaAct.DiaAct('inform(type=%s)' % goal.request_type ))
            return
        ...
        '''
        logger.debug(self.rand_decision_probs['InformToConfirm'])
        ...
        if self.use_new_goal_scenarios:
            ...
            if f < self.rand_decision_probs['ThankAck1']:
                self.push_to_agenda(DiaAct.DiaAct('thankyou()'))
                return
            elif f < self.rand_decision_probs['ThankAck1'] + self.rand_decision_probs['ThankAck2']:
                self.push_to_agenda(DiaAct.DiaAct('ack()'))
                return
        ...
        if not goal.are_all_requests_filled():
            ...
            self.push_to_agenda(user_response)
        ...
        for item in sys_act.items:
            ...
            if not goal.contains_slot_const(slot) and val != 'dontcare':
                if not Ontology.global_ontology.is_implied(self.dstring, slot=slot, value=val):
                    self.push_to_agenda(DiaAct.DiaAct('negate(%s="dontcare")' % slot))
                    return False
    ...
    def _receive_implicit_confirm(self, sys_act, goal, fromconfirm=True):
        ...
        for item in sys_act.items:
            ...
            if do_correct_misunderstanding:
                ...
                self.push_to_agenda(planned_response_act)
    ...
    def _sampleProbs(self):
        for key in self.sampling_probs:
            ...
            elif type(self.sampling_probs[key]) is dict:
                for k, v in self.sampling_probs[key].items():
                    self.rand_decision_probs[key][k] = Settings.random.uniform(v[0], v[1])
    ...
    def _read_UM_config(self, config_file_path):
        with open(config_file_path, 'r') as cf:
            ...
            if self.config.has_section('UpfrontKnowledge'):
                if self.config.has_option('UpfrontKnowledge', 'upfrontKnowledgeVectors'):
                    uk_vectors = eval(self.config.get('UpfrontKnowledge', 'upfrontKnowledgeVectors'))
                    # TODO: extend to support confidence ranges per vector element
                    self.rand_decision_probs_uk['vectors'] = []
                    for i in range(len(uk_vectors)):
                        self.rand_decision_probs_uk['vectors'].append(uk_vectors[i])

                        # store the sample probabilities per group
                        section_header = self.dstring + '_' + str(i)
                        if self.config.has_section(section_header):
                            dontknowprobs = {}
                            for option, probs in self.config.items(section_header):
                                dontknowprobs[option] = eval(probs, {}, {})
                            self.rand_decision_probs_uk['UnableToAnswer'][i] = dontknowprobs
                    self.rand_decision_probs_uk['confidence'] = eval(self.config.get('UpfrontKnowledge', 'upfrontKnowledgeCertainty'))
