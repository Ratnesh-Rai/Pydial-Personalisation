    ...
    def add_name_constraint(self, value, negate=False):
        ...
        if not negate:
            ...
            if value in wrong_venues:
                raise Exception('Failed to add name=%s: already got constraint name!=%s.' %
                             (value, value))
                return
        ...
        if negate:
            ...
            if correct_venue == value:
                raise Exception('Failed to add name!=%s: already got constraint name=%s.' % (value, value))
...
#            self.NO_REQUESTS_WITH_VALUE_NONE
    ...
    def _init_consts_requests(self, goal, um_patience):
        ...
        random_slots = list(set(random_slots))
