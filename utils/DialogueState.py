...
class DialogueState(object):
    ...
    def userActs(self, domain=None):
        if domain is None:
            return {domain: self.domainStates[domain]['userActs'] for domain in self.domainStates}
        else:
            return self.domainStates[domain]['userActs']
    ...
    def __str__(self):
        ...
        if self.currentdomain is not None:
            result += pp.pformat(self.getDomainState(self.currentdomain))
        return result
